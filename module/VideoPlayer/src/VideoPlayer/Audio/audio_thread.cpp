...
void VideoPlayer::sdlAudioCallBack(Uint8 *stream, int len){
    ...
    while(len > 0){
        ...
        if (m_bIsMute || m_bIsNeedPause) //静音 或者 是在暂停的时候跳转了
        {
            memset(audio_buf + audio_buf_index, 0, len1);
        }
        else
        {
            PcmVolumeControl::RaiseVolume((char*)audio_buf + audio_buf_index, len1, 1, m_fVolume);
        }
...
int VideoPlayer::decodeAudioFrame(bool isBlock){
    ...
    while(1){
        ...
        AVPacket packet = m_audioPacktList.front();
        ...
        {
            m_audio_clock = av_q2d(m_audioStream->time_base) * pkt->pts;
        }
        ...
        if (m_seek_flag_audio)
        {
            //发生了跳转 则跳过关键帧到目的时间的这几帧
            if (m_audio_clock < m_seek_time)
            {
                continue;
            }
            else
